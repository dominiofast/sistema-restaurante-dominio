-- 🚀 APLICAR TEMPLATE CONVERSACIONAL - TODAS AS EMPRESAS (CORRIGIDO)
...
-- 2. Aplicar template conversacional em todas as empresas (CORRIGIDO)
...
Versão: 2.0 Conversacional | Otimizada para: ' || c.name as template,
    ('{"company_name": "' || c.name || '", "company_slug": "' || c.slug || '", "business_type": "Negócio"}')::jsonb as vars,
    1 as version,
    NOW() as updated_at
FROM companies c
