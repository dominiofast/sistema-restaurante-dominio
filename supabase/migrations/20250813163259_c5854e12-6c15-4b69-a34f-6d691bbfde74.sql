-- Atualizar template para incluir lógica de delivery only
...
• Em caso de dúvida sobre pedidos já feitos, cancelamentos, trocas ou atendimento humano, diga: "Caso queira falar com Atendente diga a palavra Atendente que irei chamá-lo."',
    vars = vars || jsonb_build_object('delivery_only', false),
    vars = jsonb_set(vars, '{delivery_only}', 'false'::jsonb),
    version = version + 1,
    updated_at = now()
WHERE agent_slug IN ('quadratapizzas', 'cookielab');

-- Atualizar especificamente cookielab para ser delivery only como exemplo
UPDATE public.ai_agent_prompts 
