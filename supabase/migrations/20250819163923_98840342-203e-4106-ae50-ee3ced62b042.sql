-- CORRIGIR FUNÇÃO WHATSAPP - O PROBLEMA É A VERIFICAÇÃO DE NOTIFICAÇÃO DUPLICADA
-- Ela está verificando se JÁ ENVIOU baseado no número do pedido, mas há vários pedidos com mesmo número
...
BEGIN
    -- CORRIGIR: Verificar se já foi enviada notificação para este PEDIDO ESPECÍFICO (por ID, não número)
    SELECT EXISTS(
    ...
    IF v_notification_exists THEN
        INSERT INTO ai_conversation_logs (
            ...
            'WHATSAPP JÁ ENVIADO: Pedido ID=' || NEW.pedido_id || ' - notificação já foi enviada',
    ...
    -- Log da notificação enviada (COM IDENTIFICADOR ÚNICO)
    INSERT INTO ai_conversation_logs (
        company_id,
        customer_phone,
        customer_name,
        ...
        v_pedido_record.telefone,
        v_pedido_record.nome,
        'WHATSAPP ENVIADO COM ADICIONAIS: Pedido #' || v_pedido_record.numero_pedido || ' (pedido_id=' || NEW.pedido_id || ') confirmado - ' || v_api_response,
        'notification_sent',
        now()
    );
