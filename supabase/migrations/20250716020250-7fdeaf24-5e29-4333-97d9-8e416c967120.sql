-- Corrigir políticas RLS para usar a estrutura correta do JWT
...
-- Criar políticas corretas que usam user_metadata (não raw_user_meta_data)
...
USING (
  ...
  (company_id IN (
    ...
    WHERE c.domain = (auth.jwt() -> 'user_metadata' ->> 'company_domain') OR
