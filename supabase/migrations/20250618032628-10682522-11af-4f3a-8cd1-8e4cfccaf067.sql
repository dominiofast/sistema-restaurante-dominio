...
-- Create AI global configuration table (for super admin)
CREATE TABLE IF NOT EXISTS ai_global_config (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  openai_api_key TEXT NOT NULL,
  openai_model VARCHAR(50) DEFAULT 'gpt-4.1-2025-04-14',
...
-- Create AI agent configuration table (per company)
...
-- RLS Policies for ai_global_config (super admin only)
...
-- RLS Policies for ai_agent_config (company level)
CREATE POLICY "Companies can manage their AI agent config" ON ai_agent_config
  FOR ALL USING (
    company_id IN (
...
-- RLS Policies for ai_conversation_logs (company level for viewing)
...
CREATE TRIGGER update_ai_global_config_updated_at BEFORE UPDATE ON ai_global_config FOR EACH ROW EXECUTE PROCEDURE update_updated_at_column();
CREATE TRIGGER update_ai_agent_config_updated_at BEFORE UPDATE ON ai_agent_config FOR EACH ROW EXECUTE PROCEDURE update_updated_at_column();
