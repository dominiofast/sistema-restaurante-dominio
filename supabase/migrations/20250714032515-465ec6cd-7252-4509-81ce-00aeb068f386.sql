...
BEGIN
    ...
    -- Verificar se já enviamos notificação para este pedido (busca mais específica)
    SELECT EXISTS (
    ...
    -- Contar quantos itens tem o pedido atualmente
    SELECT COUNT(*) INTO items_count FROM pedido_itens WHERE pedido_id = NEW.pedido_id;
    
    -- Só enviar notificação se tiver pelo menos 1 item
