-- Corrigir função criar_pedido_pdv_completo para usar nome correto dos adicionais
...
BEGIN
    ...
    FOR i IN 1..array_length(v_itens_array, 1) LOOP
        ...
        -- Tratar produto_id - usar NULL se não for um UUID válido
        ...
        BEGIN
            ...
            -- Verificar se o produto existe, se não existir usar NULL
            IF NOT EXISTS (SELECT 1 FROM produtos WHERE id = v_produto_uuid) THEN
                v_produto_uuid := NULL;
            END IF;
        EXCEPTION
            WHEN invalid_text_representation THEN
                -- Se não é um UUID válido, usar NULL
                v_produto_uuid := NULL;
        END;
        
        -- Inserir item (produto_id pode ser NULL se produto não existe)
        ...
        IF v_item->'adicionais' IS NOT NULL AND json_array_length(v_item->'adicionais') > 0 THEN
            FOR v_adicional IN SELECT json_array_elements(v_item->'adicionais') LOOP
                INSERT INTO public.pedido_item_adicionais (
                    ...
                    v_item_id,
                    COALESCE(v_adicional->>'categoria', 'Adicional'),
                    COALESCE(v_adicional->>'nome', v_adicional->>'nome_adicional', v_adicional->>'name'),
                    COALESCE((v_adicional->>'quantidade')::INTEGER, (v_adicional->>'qtd')::INTEGER, 1),
                    COALESCE((v_adicional->>'preco')::NUMERIC, (v_adicional->>'valor')::NUMERIC, (v_adicional->>'price')::NUMERIC, 0),
                    COALESCE((v_adicional->>'quantidade')::INTEGER, (v_adicional->>'qtd')::INTEGER, 1) * 
                    COALESCE((v_adicional->>'preco')::NUMERIC, (v_adicional->>'valor')::NUMERIC, (v_adicional->>'price')::NUMERIC, 0)
                );
            END LOOP;
